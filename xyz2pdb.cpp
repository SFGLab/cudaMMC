...
void convert_to_pdb(std::string filename) {
    ...
    const std::string outputFileName = filename.replace(filename.length() - extensionLength, extensionLength, "pdb");
    ...
    for (const auto& point: coords) {
        ...
        outputFile << "ATOM  " << getWhiteSpaces(5 - strI.length()) << strI << "   CA ALA A" << getWhiteSpaces(4 - strI.length()) << strI << "    ";
...
void convert_to_mmcif(std::string filename) {
	...
	std::ofstream output_file(outputFileName);
		...
		output_file << "ATOM " + values[1] + " C " + values[2] + " . " + values[3] + " A  1 " + num + " ? " + values[5+i] + " " + values[6+i] + " "  + values[7+i] + " " + values[8+i] + " " + values[9+i] + " A\n";
	}
...
int main(int argc, char ** argv) {
	...
	const std::string command = "./cuMMC -a smooth -i " + hcm + " -r " + smooth_num;
    ...
    for (const auto & entry : fs::directory_iterator(in_directory)) {
	const auto tmp_path = std::string(entry.path());
	if(tmp_path.substr(tmp_path.size() - 10).compare(std::string("smooth.txt")) == 0) {
