...
__global__ void kernel(
    int * __restrict__ anchors_starts,
    int * __restrict__ anchors_ends,
    int * __restrict__ raw_arcs_starts,
    int * __restrict__ raw_arcs_ends,
    int * output_starts,
    int * output_ends,
    const int anchors_count,
    const int arcs_count
) {
    int threadIndex = blockDim.x * blockIdx.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;
    
    if (threadIndex >= arcs_count) return;
    ...
    for (int i = threadIndex; i < arcs_count; i += stride) {
        ...
        for (int j = 0; j < anchors_count; ++j) {
			if (anchor_length(anchors_starts[j], anchors_ends[j]) > 1) {
                if (anchor_contains(anchors_starts[j], anchors_ends[j], raw_arcs_starts[i])) output_starts[i] = j;
                if (anchor_contains(anchors_starts[j], anchors_ends[j], raw_arcs_ends[i])) output_ends[i] = j;
...
void InteractionArcs::parallelMarkArcs(bool ignore_missing) {
	...
	for (string chr: chrs) {
        ...
        kernel<<<blocks, threads>>>(
            ...
            thrust::raw_pointer_cast(d_outputs_end.data()),
            anchors_cnt[chr],
            cnt
        );
