  ...
  if (Settings::randomWalk) {
    ...
    if (Settings::templateSegment.size() > 1) {
      ...
      if (Settings::distHeatmap.size() > 0) {
        ...
        int tot_size = 0;
        for (string chr : chrs)
          tot_size += current_level[chr].size();
...
int LooperSolver::createTreeChromosome(string chr) {
  int cluster_start =
      clusters.size(); // index of first cluster for a given chromosome
  ...
  for (size_t i = 1; i < gaps.size(); ++i) {
    ...
    if (gaps[i] == splits[next_split_ind]) {
      ...
      int segment_cluster_ind =
          clusters.size(); // cluster-index that will be given to segment
  ...
  for (size_t i = 0; i < rootc.children.size(); ++i)
    clusters[rootc.children[i]].parent = clusters.size();
  ...
  for (string chr : chrs) {
    ...
    for (size_t i = 0; i < current_level[chr].size(); ++i)
      children_ind[chr].push_back(
          children_ind[chr][i] +
          clusters[current_level[chr][i]].children.size());
  }
...
Heatmap LooperSolver::createSingletonHeatmap(int diag) {
  ...
  for (uint fi = 0; fi < singleton_files.size(); ++fi) {
    ...
    while (!feof(f)) {
      ...
      int args_read = fscanf(f, "%s %d %d %s %d %d %d", chr1, &sta, &stb, chr2,
                             &enda, &endb, &sc);
      if (args_read < 7) {
        // this is usually true only for the last line in the file, if it is
        // empty reporting that as an error may be misleading, so only report
        // non-cmoplete and non-empty lines
        if (args_read > 0)
          cnt_lines_err++;
        continue;
      }
  ...
  for (string chr : chrs) {
    ...
    for (InteractionArc arc : arcs.long_arcs[chr]) {
      ...
      float val = Settings::longPETClustersEffectScale *
...
float LooperSolver::getHeatmapAvgBetweenNeighboringRegions(Heatmap &heat,
  ...
  for (string chr : chrs) {
    ...
    for (size_t l = 0; l < current_level[chr].size(); ++l)
      v.push_back(v[l] + clusters[current_level[chr][l]].children.size());
    n = v.size();
  ...
  for (auto el : active_anchors_neighbors) {
    ...
    for (int i = 0; i < n; ++i) {
      ang = angle_norm(orientation[el.first],
                       (Settings::motifsSymmetric ? 1.0 : -1.0) *
  ...
  for (int val : active_anchors_neighbors[anchor_index]) {
    ang =
        angle_norm(orientation[anchor_index],
                   (Settings::motifsSymmetric ? 1.0 : -1.0) * orientation[val]);
    err += ang * ang;
  }
...
double LooperSolver::calcScoreHeatmapActiveRegion(int moved) {
  ...
  if (moved == -1) {
    for (size_t i = 0; i < n; ++i)
      err += calcScoreHeatmapActiveRegion(i);
  } else {
    int st = 0;
    int end = n - 1;
...
float LooperSolver::freqToDistance(int freq, bool memo) {
  ...
  return Settings::countToDistBaseLevel +
...
void LooperSolver::reconstructClusterArcsDistances(int cluster, int cluster_ind,
  ...
  for (int k = 0; k < steps; ++k) {
    ...
    score = smooth ? MonteCarloArcsSmooth(noise_size, use_subanchor_heatmap)
                   : MonteCarloArcs(noise_size);
      ...
      for (size_t j = 0; j < n; ++j) {
        ...
        if (pos < center_loc) {
          ...
          if (switch_controls == -1)
            switch_controls = j; // this was the first child to cross 1.0
        }
        knots.push_back(p);
      }
    } else {
...
double LooperSolver::MonteCarloArcsSmooth(float step_size,
  ...
  // if (!silent) for (i=0; i<anchor_orientation.size(); i++)
  // print_vector(anchor_orientation[i], "orient");
  return score_curr;
}
  ...
  for (string chr : chrs) {
    for (size_t i = 0; i < hc.chr[chr].points.size(); ++i) {
      for (size_t j = 0; j < dim; ++j) {
        if (hc.chr[chr].points[i][j] < min[j])
          min[j] = hc.chr[chr].points[i][j];
        if (hc.chr[chr].points[i][j] > max[j])
          max[j] = hc.chr[chr].points[i][j];
      }
    }
  }
...
void LooperSolver::showCoordinatesRange(bool use_active_region) {
  ...
  if (use_active_region) {
    ...
    for (size_t i = 0; i < active_region.size(); ++i) {
      for (size_t j = 0; j < dim; ++j) {
        if (clusters[active_region[i]].pos[j] < min[j])
          min[j] = clusters[active_region[i]].pos[j];
        if (clusters[active_region[i]].pos[j] > max[j])
          max[j] = clusters[active_region[i]].pos[j];
      }
    }
...
void LooperSolver::calcActiveAnchorsNeighbors() {
  ...
  for (size_t i = 0; i < anchors.size(); ++i) {
    ...
    for (size_t j = 0; j < clusters[ai].arcs.size(); ++j) {
      ...
      active_anchors_neighbors[i].push_back(
